...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (!message.member.roles.cache.has(`${mainconfig.SeverRoles.OwnerRoleId}`)) return message.reply({
            content: `<:no:1079212019376476160> **You Are Not Allowed To Execute This Command, Only Owners!**`
        })
        try {
            ...
            if (!bot || !bot.id) return message.channel.send("**<:no:1079212019376476160> To Delete The Folder, Please Mention a Bot!**");
            ...
            collector.on("collect", (b) => {
                if (b.user.id !== message.author.id)
                    return b.reply({
                        content: `<:no:1079212019376476160> **Only <@${message.author.id}> Allowed To React To This Message!**`,
                        ephemeral: true
                    })
                if (b.isButton()) {
