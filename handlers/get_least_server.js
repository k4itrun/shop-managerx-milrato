...
const config_channel = require(`${process.cwd()}/config/config.json`)
const {
    Client
} = require('ssh2');
const {
...
module.exports = client => {
    const nodestatChannelID = mainconfig.nodeStatsChannelID;
    const nodestatMessageID = mainconfig.nodeStatsMessageID;
   ...
   //     client.allServers.stats = sorted;
        ...
        client.channels.fetch(nodestatChannelID).then(channel => {
            channel.messages.fetch(nodestatMessageID).then(msg => {
                ...
                const online = "<a:Online_Ping:1075486861960679425>";
                const offline = "<a:offline:1075488365010505819>";
                const embed = new Discord.MessageEmbed()
                    .setColor(client.config.color)
                    .setAuthor("Team Arcades | Nodestats", msg.guild.iconURL({ dynamic: true }), "https://discord.gg/team-arcades-935157109761388554")
                    .setFooter("teamarcades.xyz | Order Bots NOW\nThis Message get's updated every 15 Minutes", "https://i.imgur.com/UnHkdfP.png")
