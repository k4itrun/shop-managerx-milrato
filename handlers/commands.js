...
const {
    Roles
} = require(`${process.cwd()}/config/settings.json`);
const {
    readdirSync
} = require("fs");
const {
...
const rules = require(`${process.cwd()}/config/rules.js`);
        ...
        if (cmd == "nodestats") {
            ...
            if (message.member.permissions.has("ADMINISTRATOR")) {
                let embed = new Discord.MessageEmbed()
                    ...
                    .setColor(client.config.color)
                let menuoptions = require(`${process.cwd()}/config/settings.json`).ticketsystem;
...
<:Aracades_X_Rythm:1079545576691093614> **__Rythm__ Clone** 🎵 | <@1059510357120536587>
...
.setImage(`${mainconfig.OrdersChannelID.ImgOrder}`)
                    .setColor(client.config.color)
                let menuoptions = require(`${process.cwd()}/config/settings.json`).ordersystem;
                //define the selection
                let Selection = new MessageSelectMenu()
...
<:arrow:1079211960199028796> ***Make a selection to see the characteristics of each bot!***`).setColor(client.config.color)
                let menuoptions = require(`${process.cwd()}/config/settings.json`).features;
                //define the selection
                let Selection = new MessageSelectMenu()
            ...
            if (message.member.permissions.has("ADMINISTRATOR")) {
                var {
                    SystemRolesMenu 
                } = require(`${process.cwd()}/config/mainconfig.js`)
                let id = args[0],
                    themessage_ = false;
                if (id) {
                    themessage_ = await message.channel.messages.fetch(id).catch(() => { }) || false;
                ...
                //values
                let menuoptions = require(`${process.cwd()}/config/settings.json`).selectRoles;
                //define the selection
                let Selection = new MessageSelectMenu()
         ...
         */
            ...
            let menuoptions = require(`${process.cwd()}/config/settings.json`).createbot
            //define the selection
            let Selection = new MessageSelectMenu()
