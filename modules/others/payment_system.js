      ...
      for (const botdelete of botdeletes) {
        try {
          ...
          if (!botdelete.sent) {
            ...
            await ch.send({content: `<:leave:1079212004096622647> **${member.user} | ${member.user.tag} (\`${member.user.id}\`) Bots can be deleted, he/she left the Server!!**\n__The Bot:__\n> ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)`, 
				embeds: [
      ...
      for (const botdelete of botdeletes) {
        try {
          ...
          if (!botdelete.sent) {
            ...
            var bots = client.bots.get(member.id, "bots")
            stopBot(client, botdelete, bot, member)
            await ch.send({content: `<a:Money_Price:1075487970339078285> **${member.user} | ${member.user.tag} (\`${member.user.id}\`) needs to Pay after ${duration(botdelete.time).map(i=>`\`${i}\``).join(" ")} (Payed at: <t:${Math.floor((Date.now() - botdelete.time) / 1000)}>) again!**\n__The Bot:__\n> ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)`,
        ...
        for (const botdelete of botdeletes) {
          try {
            ...
            if (!botdelete.sent) {
              ...
              var bots = client.bots.get(member.id, "bots");
              stopBot(client, botdelete, bot, member)
              await ch.send({content: `<a:Server_Boosts:1079211967681659000> **${member.user} | ${member.user.tag} (\`${member.user.id}\`) Bots can be deleted, he/she stopped boosting us!!**\n__The BOOSTPAYMENT-Bot:__\n> ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)`, 
                embeds: [new Discord.MessageEmbed().setColor("RED").setDescription(`${botdelete.data}`).setAuthor(bot.user.tag + " - Is Deleteable", bot.user.displayAvatarURL())
                  .addField("__ALL OF HIS/HER BOTS:__", ">>> " + bots.length > 0 ? bots.length <= 10 ? bots.map(bot => `**${client.bots.get(bot, "type")}** | <@${bot}>`).join("\n") : bots.slice(0, 10).map(bot => `**${client.bots.get(bot, "type")}** | <@${bot}>`).join("\n") + bots.length - 10 + " More ..." : "He Doesn't Have Any Bots Yet!")
                  .setFooter(member.user.id, member.user.displayAvatarURL({
                    dynamic: true
                      ...
                      if(alldata){
                        ...
                        try{
                          client.channels.fetch("964370139225145485").catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)}).then(ch=>{
                            ch.send({content: `<a:check:1079215644274335834> **Deleted the Bot:** ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)\n> **BOT-ID:** \`${botid}\`, **Server:** \`${serverid}\``})
                            try{
                              member.send({content: `<a:check:1079215644274335834> **Stopped your Bot:** ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)\n> **This is because you need to pay again / stopped paying us / left the Server!**`})
                            }catch (e){
                              console.log(e)
                            }
                          })
                        ...
                        try{
                          client.channels.fetch("964370139225145485").catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)}).then(ch=>{
                            ch.send({content: `<:no:1079212019376476160> **Unable to Stop the Bot:** ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)`})
                          })
                        }catch (e){
                          console.log(e)
                        }
